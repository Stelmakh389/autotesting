...
class Equipment(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['zav_nomer'],
                condition=~Q(zav_nomer__in=['', '-', 'б/н']) & ~Q(zav_nomer__isnull=True),
                name='unique_zav_nomer'
            )
        ]
    
    def save(self, *args, **kwargs):
        # Нормализация значений заводского номера
        if self.zav_nomer:
            self.zav_nomer = self.zav_nomer.strip()
            if self.zav_nomer.lower() == 'б/н':
                self.zav_nomer = 'б/н'
    ...
    def days_between_poverk(self):
        ...
        if self.srok_poverk:
            # Используем date.today() вместо localdate() для стандартизации расчета
            return (self.srok_poverk - date.today()).days
        return None
